    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-slate-900 rounded-lg shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            {variables.length === 0 ? (
              <div className="text-center py-8 text-slate-500">
                ...
                <p className="text-sm mt-2">
                  Use <code className="bg-white/10 px-1 rounded">{'{{variable}}'}</code> in prompts to add inputs.
                </p>
              </div>
            ) : (
                ...
                {variables.map((varName) => (
                  <div key={varName}>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      This will replace <code className="bg-white/10 px-1 rounded">{`{{${varName}}}`}</code> in the workflow
                    </p>
                  </div>
                ))}
              </div>
          ...
          {/* Footer */}
          <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/10 bg-white/[0.03]">
            <button
              type="button"
              onClick={onClose}
